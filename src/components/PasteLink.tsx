...
const PasteLink = () => {
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      ...
      if (isToken) {
        response = await fetch(
          "https://urlshortener-backend-production.up.railway.app/api/url_shortener",
          {
            method: "POST",
            headers: {
    ...
    if (!isToken) setValue("title", "null");
  ...
  return (
    <>
      <div
        className={`${isShortened ? "hidden" : "flex flex-col sm:w-[35rem]"}`}
      >
        <form onSubmit={handleSubmit(onSubmit)} className="">
          <div className="flex flex-col gap-2">
            <div className="flex flex-row items-center justify-between  bg-white rounded-full gap-2 drop-shadow-lg">
              <label htmlFor="pasteLink">
                <HiOutlineLink
                  style={{ color: "#766FF9" }}
                  size={20}
                  className="ml-5 sm:ml-4 cursor-pointer"
                />
              </label>
              <input
                id="pasteLink"
                type="text"
                placeholder="Paste a link and shorten it"
                {...register("long_url")}
                className="grow text-sm py-4 px-2 sm:py-3 sm:text-base focus:outline-none"
              />
              <input
                type="submit"
                value="Shorten"
                className="py-2 px-5 mr-3 cursor-pointer bg-[#766FF9] hover:bg-[#041267] duration-500 ease-in-out rounded-full shadow-lg text-white text-sm"
              />
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-2 mx-5">
              <div className="grow-0 flex flex-col gap-2 basis-[60%]">
                <Switch
                  name="is_custom"
                  toggle={{
                    isToggle: customIsToggled,
                    setIsToggle: setCustomIsToggled,
                  }}
                  textReg="short_url"
                  register={register}
                  errors={errors}
              ...
              <div className="grow-0 flex flex-col gap-2 basis-[40%]">
                <Switch
                  name="is_private"
                  toggle={{
                    isToggle: privateIsToggled,
                    setIsToggle: setPrivateIsToggled,
                  }}
                  textReg="password"
                  register={register}
                  errors={errors}
            ...
            <div
              className={` ${isToken ? "flex flex-col gap-2 mx-5" : "hidden"}`}
            >
              <Switch
                name="is_feeds"
                toggle={{
                  isToggle: feedsIsToggled,
                  setIsToggle: setFeedsIsToggled,
                }}
                register={register}
                errors={errors}
                label1="Feeds"
            ...
            <div className={` ${isToken ? "flex flex-col gap-2" : "hidden"}`}>
              ...
              <p className="text-sm text-red-500 font-medium mx-5">
      ...
      {isShortened && (
        <div className="flex flex-col items-center gap-4">
          <div className="flex flex-row items-center justify-between bg-white rounded-full py-2 px-3 hover:bg-gray-200 duration-500 ease-in-out max-w-[20rem] sm:max-w-[35rem]">
            <label
              htmlFor="copy"
          ...
          <input
            type="button"
            value="Shorten More Link!"
            onClick={newLink}
            className="py-2 px-5 cursor-pointer bg-[#766FF9] hover:bg-[#041267] duration-500 ease-in-out  rounded-full shadow-lg text-white text-sm"
          />
        </div>
      )}
